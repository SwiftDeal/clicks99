                ...
                if (!location_cookie && location_cookie.source === 'ipinfodb') {
                    win.ipinfocb = function(data) {
                        if (data.statusCode === "OK") {
                            ...
                            if (options.gapi_location) {
                                return modules.gapi_location()(callback); } else { callback({ error: true, source: "ipinfodb", message: data.statusMessage }); }
                        }
                    };
...
(function(window, $) {
    ...
    Track = (function($) {
        function Track() {
            this.domain = 'adtrack.clicks99.com';
        }
        Track.prototype = {
            pageview: function() {
                var self = this;
                $.ajax({
                        url: 'http://' + self.domain + '/process.php',
                        type: 'GET',
                        async: true,
                        data: {},
                    })
